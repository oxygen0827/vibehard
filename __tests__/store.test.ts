...
describe("tenant isolation", () => {
  ...
  it("只解析平台已启用且 provider 匹配的模型", async () => {
    await expect(resolveModelProfile("gpt-5.6-sol", "tokenadvent")).resolves.toMatchObject({ providerId: "tokenadvent" });
    await expect(resolveModelProfile("gpt-5.6-sol", "unknown")).resolves.toBeNull();
  ...
  it("同一会话同时只能创建一个活跃任务", async () => {
    ...
    const results = await Promise.allSettled([
      createTurn(owner.id, thread.id, "first", "gpt-5.6-sol", "tokenadvent"),
      createTurn(owner.id, thread.id, "second", "gpt-5.6-sol", "tokenadvent"),
    ]);
