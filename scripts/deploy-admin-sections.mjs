...
const release = "/opt/vibehard/releases/20260919-admin-sections";
const previous = "/opt/vibehard/releases/20260918-llm-settings";
...
if (mode === "preflight") {
  ...
  run("systemd-run", ["--unit=vibehard-admin-sections-preflight", "--property=Type=simple", `--property=WorkingDirectory=${release}/standalone`, "--property=EnvironmentFile=/etc/vibehard/platform.env", "--setenv=NODE_ENV=production", "--setenv=HOSTNAME=127.0.0.1", "--setenv=PORT=3211", node, "server.js"]);
  await ready(3211);
  verify(3211);
  console.log("Admin sections preflight passed on localhost:3211");
}
if (mode === "rollback") {
...
if (mode === "activate") {
  ...
  try {
    ...
    run("systemctl", ["stop", "vibehard-admin-sections-preflight.service"]);
    console.log("Activated admin sections; Gateway, VibeBoard and nginx were preserved");
  } catch (error) {
    copyFileSync(`${backup}/vibehard.service`, unit);
    run("systemctl", ["daemon-reload"]);
    run("systemctl", ["restart", "vibehard.service"]);
