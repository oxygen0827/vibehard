...
const release = "/opt/vibehard/releases/20260919-auth-cookies";
const previous = "/opt/vibehard/releases/20260919-admin-sections";
...
if (mode === "preflight") {
  ...
  run("systemd-run", ["--unit=vibehard-auth-cookies-preflight", "--property=Type=simple", `--property=WorkingDirectory=${release}/standalone`, "--property=EnvironmentFile=/etc/vibehard/platform.env", "--setenv=NODE_ENV=production", "--setenv=HOSTNAME=127.0.0.1", "--setenv=PORT=3211", node, "server.js"]);
  await ready(3211);
  verify(3211);
  console.log("Auth cookies preflight passed on localhost:3211");
}
if (mode === "rollback") {
...
if (mode === "activate") {
  ...
  try {
    ...
    run("systemctl", ["stop", "vibehard-auth-cookies-preflight.service"]);
    console.log("Activated auth cookies; Gateway, VibeBoard and nginx were preserved");
  } catch (error) {
    copyFileSync(`${backup}/vibehard.service`, unit);
    run("systemctl", ["daemon-reload"]);
    run("systemctl", ["restart", "vibehard.service"]);
