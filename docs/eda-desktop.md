...
2026-09-25：`/eda` 已切换为实际 KiCad 桌面，并在右侧加入“原理图 Agent”对话入口；`/eda/legacy` 保留原网页草稿编辑器。功能分支 `codex/web-eda-workbench`，尚未部署生产。新建工程为空白，没有预置示例电路。本机尚未配置可用的设计模型，因此对话发送按钮会如实禁用，真实 AI 生成尚未验收。
...
- 点击顶部“Agent 画图”可收起或展开右侧对话。管理员需先在“模型设置”配置设计模型；页面显示真实可用状态。描述电路后，审阅命令清单和器件/网络草稿，再点“加入设计草稿”及“创建并打开 KiCad 工程”。这条链路需要真实设计模型才能完成端到端验收。
...
1. Agent 已有对话入口、受控提案与确认后新建原生工程的路径，但本机设计模型未配置，真实生成链路仍未验收；目前不支持就地修改当前 KiCad 工程，也没有原生文件差异预览、版本冲突处理或全器件库覆盖。旧 Agent 仍服务旧版文档，不能声称它会控制新 KiCad 会话。
...
下一阶段优先接通可靠设计模型并完成真实 AI → 提案 → 原生文件 → KiCad 的端到端验收；扩展器件库和无损导入后，再实现已保存原生工程的差异预览、检查、版本冲突处理和受控就地应用。
