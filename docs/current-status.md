...
| --- | --- | --- |
| `cloud-runner` | 长期在线对话、代码生成、原生 C/C++ 编译、工程下载 | online，生产真实任务已通过 |
| `device-runner` | Mac mini 本地 USB、串口、烧录和实机日志 | online，尚未接入小电脑基础工程和实机 |
| `mac-local` | 旧开发 Runner | offline，不再作为默认节点 |

云端链路已经不依赖 Mac mini。需要实体数据线的任务必须选择 `device-runner`，并在现场 Mac mini 在线、产品工具链与设备均已准备的情况下运行。
...
1. 发布新建项目的执行器选择页面，分别验证 `cloud-runner` 和 `device-runner` 项目路由。
2. 接收“小电脑”基础工程、产品知识库、构建／烧录／日志命令，在设备 Runner 工作区中落盘。
3. 连接真实数据线，验证设备识别、编译、烧录、重启、日志回传和失败恢复。
4. 用两个固定 APP 做至少三轮全链路彩排，记录成功率和剩余人工步骤。
