...
同伴及其 Agent **只向 GitHub 提交可审查的交付包，不直接向生产服务器上传**。平台代码经版本化 release 发布；项目知识经项目 ID 解析后导入该项目唯一 workspace；大文件进入对象存储。禁止把文件散落到 active release、Runner 会话目录或旧 `incoming` 目录。
